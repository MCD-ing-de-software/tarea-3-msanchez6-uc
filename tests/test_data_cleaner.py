...
class TestDataCleaner(unittest.TestCase):
    ...
    def test_drop_invalid_rows_removes_rows_with_missing_values(self):
        ...
        missing_counts = result[["name", "age"]].isna().sum()
        expected_missing_counts = pd.Series([0,0], index=["name", "age"])
        pdt.assert_series_equal(missing_counts, expected_missing_counts)

        is_smaller = pd.Series([len(result) < len(df)], name="is_smaller")
        expected_is_smaller = pd.Series([True], name="is_smaller")
        pdt.assert_series_equal(is_smaller, expected_is_smaller)
    ...
    def test_drop_invalid_rows_raises_keyerror_for_unknown_column(self):
        ...
        with self.assertRaises(KeyError):
            cleaner.drop_invalid_rows(df, ["does_not_exit"])

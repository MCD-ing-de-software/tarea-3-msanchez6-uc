...
class TestStatisticsUtils(unittest.TestCase):
    ...
    def test_zscore_has_mean_zero_and_unit_std(self):
        ...
        self.assertAlmostEqual(float(np.mean(result)), 0.0, places=7)
        self.assertAlmostEqual(float(np.std(result)), 1.0, places=7)
    ...
    def test_min_max_scale_maps_to_zero_one_range(self):
        ...
        self.assertAlmostEqual(float(np.min(result)), 0.0, places = 7)
        self.assertAlmostEqual(float(np.max(result)), 1.0, places = 7)
        npt.assert_allclose(result, expected, rtol=1e-7, atol=1e-7)
